...
module "coordinator" {
  ...
  environment = merge(local.common_environment, {
    ...
    CONTROLGRAPH_ISSUER_URL            = module.issuer.service.uri
    CONTROLGRAPH_VERIFIER_URL          = module.verifier.service.uri
    CONTROLGRAPH_EXECUTOR_URL          = module.executor.service.uri
    CONTROLGRAPH_RECOVERY_URL          = module.recovery.service.uri
...
module "api" {
  ...
  environment = merge(local.common_environment, {
    ...
    CONTROLGRAPH_COORDINATOR_URL        = module.coordinator.service.uri
    CONTROLGRAPH_CAPABILITY_KEY_VERSION = data.terraform_remote_state.foundation.outputs.signing_keys.capability.version
