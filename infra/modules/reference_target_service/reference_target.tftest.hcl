...
run "stable_revision_starts_at_the_bounded_baseline" {
  ...
  assert {
    condition = (
      ...
      google_cloud_run_v2_service.reference.template[0].revision == "controlgraph-reference-target-stable-v10" &&
  ...
  assert {
    condition = (
      ...
      length([
        ...
        if target.revision == "controlgraph-reference-target-stable-v10" &&
  ...
  assert {
    condition = (
      ...
      output.target.baseline_reset.stable_revision == "controlgraph-reference-target-stable-v10" &&
      output.target.baseline_reset.candidate_revision == "controlgraph-reference-target-candidate-v10" &&
...
run "candidate_configuration_shape_is_bounded" {
  ...
  assert {
    condition = (
      google_cloud_run_v2_service.reference.template[0].revision == "controlgraph-reference-target-candidate-v10" &&
  ...
  assert {
    condition = (
      ...
      length([
        for target in google_cloud_run_v2_service.reference.traffic : target
        if target.revision == "controlgraph-reference-target-candidate-v10" &&
