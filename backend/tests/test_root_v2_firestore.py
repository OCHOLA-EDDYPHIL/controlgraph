...
import pytest
from root_v2_test_data import RootV2Records, make_root_v2_records, root_v2_target
...
def test_root_v2_bundle_is_one_six_record_commit_and_coherent_read() -> None:
    async def scenario() -> None:
        ...
        expected_paths = {
            f"{AuthorityStorageKind.ROLLOUT_ROOT_V2.value}/"
            f"{rollout_root_v2_document_id(records.root.root_id)}",
            ...
            f"{AuthorityStorageKind.ROOT_CREATION_RESULT.value}/"
            f"{root_creation_result_document_id(records.root.root_id)}",
        }
...
def test_competing_root_creation_has_one_claim_winner() -> None:
    async def scenario() -> None:
        ...
        first = make_root_v2_records(variant=1)
        second = make_root_v2_records(variant=2)
        ...
        root_paths = [
            path
            for path in client.documents
            if path.startswith(f"{AuthorityStorageKind.ROLLOUT_ROOT_V2.value}/")
        ]
...
def test_ambiguous_complete_commit_adopts_but_partial_commit_fails_closed() -> None:
    async def scenario() -> None:
        ...
        partial_records = make_root_v2_records(variant=2)
...
def test_coherent_read_rejects_a_missing_or_recombined_bundle_record() -> None:
    async def scenario() -> None:
        ...
        absent = make_root_v2_records(variant=2)
