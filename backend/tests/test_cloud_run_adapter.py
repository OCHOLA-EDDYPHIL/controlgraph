...
@pytest.mark.parametrize(
    ...
    [
        ...
        replace(
            ...
            stable_revision=f"{SERVICE}-stable-v19",
        ),
        replace(
            target_configuration_projection(
                _verified().request.intent,
            ...
            candidate_revision=f"{SERVICE}-candidate-v19",
        ),
        replace(
            target_configuration_projection(
                _verified().request.intent,
...
def test_target_configuration_projection_and_digest_are_stable() -> None:
    ...
    assert target_configuration_sha256(intent, expected_concurrency=8) == (
        "e767ac667449fb48572ec5a33101a0c17e0ba14a2cfe737f486db6bcc6144e41"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"stable_revision": f"{SERVICE}-stable-v19"}, 8),
        ({"candidate_revision": f"{SERVICE}-candidate-v19"}, 8),
...
@_async_test
async def test_reference_target_reset_migrates_the_exact_v17_baseline_to_v18() -> None:
    before = _service(
        100,
        0,
        ...
        generation=8,
        latest_ready_revision=(
            f"{SERVICE_RESOURCE}/revisions/controlgraph-reference-target-candidate-v17"
        ),
        latest_created_revision=f"{SERVICE_RESOURCE}/revisions/{CANDIDATE}",
    )
...
@_async_test
async def test_reference_target_reset_rejects_any_other_v17_traffic_shape(
    case: str,
) -> None:
    before = _service(
        100,
