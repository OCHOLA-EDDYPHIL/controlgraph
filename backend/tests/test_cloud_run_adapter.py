...
@pytest.mark.parametrize(
    ...
    [
        ...
        replace(
            ...
            stable_revision=f"{SERVICE}-stable-v14",
        ),
        replace(
            target_configuration_projection(
                _verified().request.intent,
            ...
            candidate_revision=f"{SERVICE}-candidate-v14",
        ),
        replace(
            target_configuration_projection(
                _verified().request.intent,
...
def test_target_configuration_projection_and_digest_are_stable() -> None:
    ...
    assert target_configuration_sha256(intent, expected_concurrency=8) == (
        "eb70c017d51329fdd563ca8db58c00afe84d044b39593af5b6e426a26d78e4a2"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"stable_revision": f"{SERVICE}-stable-v14"}, 8),
        ({"candidate_revision": f"{SERVICE}-candidate-v14"}, 8),
...
@_async_test
async def test_reference_target_reset_migrates_the_exact_v12_baseline_to_v13() -> None:
    before = _service(
        100,
        0,
        ...
        generation=8,
        latest_ready_revision=(
            f"{SERVICE_RESOURCE}/revisions/controlgraph-reference-target-candidate-v12"
        ),
        latest_created_revision=f"{SERVICE_RESOURCE}/revisions/{CANDIDATE}",
    )
...
@_async_test
async def test_reference_target_reset_rejects_any_other_v12_traffic_shape(
    case: str,
) -> None:
    before = _service(
        100,
