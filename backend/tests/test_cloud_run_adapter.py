...
STABLE = f"{SERVICE}-stable-v16"
CANDIDATE = f"{SERVICE}-candidate-v16"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        replace(
            ...
            stable_revision=f"{SERVICE}-stable-v17",
        ),
        replace(
            target_configuration_projection(
                _verified().request.intent,
            ...
            candidate_revision=f"{SERVICE}-candidate-v17",
        ),
        replace(
            target_configuration_projection(
                _verified().request.intent,
...
def test_target_configuration_projection_and_digest_are_stable() -> None:
    ...
    assert target_configuration_sha256(intent, expected_concurrency=8) == (
        "99b851ea52fa4860029a65a9568ef1675b36a0958d792903b48e339893e88b35"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"stable_revision": f"{SERVICE}-stable-v17"}, 8),
        ({"candidate_revision": f"{SERVICE}-candidate-v17"}, 8),
...
@_async_test
async def test_reference_target_reset_migrates_the_exact_v15_baseline_to_v16() -> None:
    before = _service(
        100,
        0,
        ...
        generation=8,
        latest_ready_revision=(
            f"{SERVICE_RESOURCE}/revisions/controlgraph-reference-target-candidate-v15"
        ),
        latest_created_revision=f"{SERVICE_RESOURCE}/revisions/{CANDIDATE}",
    )
...
@_async_test
async def test_reference_target_reset_rejects_any_other_v15_traffic_shape(
    case: str,
) -> None:
    before = _service(
        100,
