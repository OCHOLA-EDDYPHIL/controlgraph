...
@pytest.mark.parametrize(
    ...
    [
        ...
        replace(
            ...
            stable_revision=f"{SERVICE}-stable-v13",
        ),
        replace(
            target_configuration_projection(
                _verified().request.intent,
            ...
            candidate_revision=f"{SERVICE}-candidate-v13",
        ),
        replace(
            target_configuration_projection(
                _verified().request.intent,
...
def test_target_configuration_projection_and_digest_are_stable() -> None:
    ...
    assert target_configuration_sha256(intent, expected_concurrency=8) == (
        "490a9ba14fd6598460c6a6deeb070eac8c11929799c640475bed9ec640297641"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"stable_revision": f"{SERVICE}-stable-v13"}, 8),
        ({"candidate_revision": f"{SERVICE}-candidate-v13"}, 8),
...
@_async_test
async def test_reference_target_reset_migrates_the_exact_v11_baseline_to_v12() -> None:
    before = _service(
        100,
        0,
        ...
        generation=8,
        latest_ready_revision=(
            f"{SERVICE_RESOURCE}/revisions/controlgraph-reference-target-candidate-v11"
        ),
        latest_created_revision=f"{SERVICE_RESOURCE}/revisions/{CANDIDATE}",
    )
...
@_async_test
async def test_reference_target_reset_rejects_any_other_v11_traffic_shape(
    case: str,
) -> None:
    before = _service(
        100,
