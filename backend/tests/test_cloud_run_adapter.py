...
def test_target_configuration_projection_and_digest_are_stable() -> None:
    ...
    assert target_configuration_sha256(intent, expected_concurrency=8) == (
        "6b55007cb18f6c4506aac05f0009298a337a54f32c78b59efc3652a2e8d7e4ff"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"stable_revision": f"{SERVICE}-stable-v18"}, 8),
        ({"candidate_revision": f"{SERVICE}-candidate-v18"}, 8),
...
@_async_test
async def test_reference_target_reset_migrates_the_exact_v16_baseline_to_v17() -> None:
    before = _service(
        100,
        0,
        ...
        generation=8,
        latest_ready_revision=(
            f"{SERVICE_RESOURCE}/revisions/controlgraph-reference-target-candidate-v16"
        ),
        latest_created_revision=f"{SERVICE_RESOURCE}/revisions/{CANDIDATE}",
    )
...
@_async_test
async def test_reference_target_reset_rejects_any_other_v16_traffic_shape(
    case: str,
) -> None:
    before = _service(
        100,
