...
def test_target_configuration_projection_and_digest_are_stable() -> None:
    ...
    assert target_configuration_sha256(intent, expected_concurrency=8) == (
        "8342617835972e2dc738ce5ffa1b062e7f55b8a2e594ff9547c7c3f7d87fb572"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"stable_revision": f"{SERVICE}-stable-v11"}, 8),
        ({"candidate_revision": f"{SERVICE}-candidate-v11"}, 8),
...
@_async_test
async def test_reference_target_reset_migrates_the_exact_v9_baseline_to_v10() -> None:
    before = _service(
        100,
        0,
        ...
        generation=8,
        latest_ready_revision=(
            f"{SERVICE_RESOURCE}/revisions/controlgraph-reference-target-candidate-v9"
        ),
        latest_created_revision=f"{SERVICE_RESOURCE}/revisions/{CANDIDATE}",
    )
...
@_async_test
async def test_reference_target_reset_rejects_any_other_v9_traffic_shape(
    case: str,
) -> None:
    before = _service(
        100,
